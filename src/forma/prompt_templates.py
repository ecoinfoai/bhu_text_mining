...
RUBRIC_SYSTEM_INSTRUCTION = (
    ...
    "이 답변은 자필 작성 후 이미지에서 인식된 텍스트이므로 오탈자를 맥락상 이해하여 평가하세요. "
...
FEEDBACK_SYSTEM_INSTRUCTION = (
    ...
    "세 단락 모두 반드시 포함해야 하며, 어떤 단락도 생략하지 마세요. "
    "각 단락은 반드시 대괄호로 감싼 제목(예: [현재 상태])으로 시작해야 합니다. "
    "각 단락은 2~3문장으로 간결하게 작성하세요. "
    "[학생에게 권하는 사항]은 점수와 무관하게 항상 작성하세요. "
    "부족한 부분이 없는 학생에게는 심화 학습 방향이나 응용 연습을 제안하세요. "
    "마크다운 문법(#, **, *, ```)을 사용하지 마세요. "
...
## 평가 지침
- 이 답변은 자필 작성 후 이미지에서 인식된 텍스트입니다. 글씨체 특성이나 OCR 인식 오류로 인한 오탈자가 포함될 수 있습니다.
...
## 지침
- 이 답변은 자필 작성 후 이미지에서 인식된 텍스트입니다. 오탈자가 포함될 수 있으므로, 맥락상 의미를 파악하여 마스터 노드와 매칭하세요.
...
## 피드백 작성 지침
1. 이 답변은 자필 작성 후 이미지에서 인식된 텍스트입니다. 오탈자가 포함될 수 있으므로 맥락을 고려하여 피드백하세요.
