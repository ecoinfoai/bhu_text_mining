...
class TestTranscriptPattern:
    ...
    def test_no_transcript_pattern_no_emphasis_processing(self, tmp_path, monkeypatch):
    ...
    def test_transcript_pattern_nonexistent_dir_logs_warning(self, tmp_path, monkeypatch, caplog):
        ...
        monkeypatch.setattr(
            ...
            [
                ...
                "--no-individual",
                "--transcript-pattern",
                str(tmp_path / "no_such_{class}"),
            ],
        )

        with (
            caplog.at_level(logging.WARNING, logger="forma.cli_report_batch"),
            patch(
                "forma.cli_report_batch.load_all_student_data",
                return_value=(_make_mock_students(5), MagicMock()),
            ),
    ...
    def test_transcript_pattern_class_substitution(self, tmp_path, monkeypatch):
        ...
        (transcript_dir_A / "lecture.txt").write_text("심장은 혈액을 순환시키는 기관이다.", encoding="utf-8")
