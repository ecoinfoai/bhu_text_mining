...
### Step 0: Create the Week Directory and week.yaml

Each assessment week lives in its own directory. A `week.yaml` file drives the full scan → eval pipeline via `{class}` pattern substitution, eliminating the need to repeat long paths on every command.

**Create the week directory and write week.yaml:**
...
```yaml
# week.yaml — place this file in the week directory
week: 1
...
ocr:
  ...
  crop_coords: []   # auto-populated on first scan run
...
With `week.yaml` in place, all subsequent commands auto-resolve paths from the file:

```bash
forma select                  # generate questions.yaml + exam PDF
forma ocr scan --class A      # resolves {class}=A from week.yaml
forma ocr join --class A
forma eval --class A
```

See [Configuration](configuration.md#weekyaml) for the full `week.yaml` field reference.
...
```bash
forma report student --final results/final_A.yaml --config exams/week_N.yaml \
...
```bash
forma report student --final final.yaml --config exam.yaml --eval-dir eval/ \
